...
# Modified
    ...
    def load_local(
        ...
        **kwargs: Any,
        ...
        faiss_pth = path / f"{index_name}.faiss"
        pkl_pth = path / f"{index_name}.pkl"

        if not faiss_pth.exists():
            raise FileNotFoundError(f"Missing FAISS index file: {faiss_pth}")
        if not pkl_pth.exists():
            raise FileNotFoundError(f"Missing pickle file: {pkl_pth}")
        # s = time.time()

        faiss = dependable_faiss_import()
        index = faiss.read_index(str(faiss_pth))

        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(pickle.load, pkl_pth.open("rb"))
            docstore, index_to_docstore_id = future.result()
